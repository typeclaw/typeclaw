...
It is **not** the mechanism for keeping a public-facing review away from private code and credentials. That separation is coarse — it wants a whole-container, whole-credential-set boundary — and typeclaw already provides it: one agent is one container, one `.env`, one `GH_TOKEN`, one `/agent`. The right boundary is operational.
