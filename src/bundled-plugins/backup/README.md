...
| `backup.gcPackThreshold`  | `20`    | After a successful commit, run `git gc` when the repo's pack count reaches this many. Keeps object/pack growth bounded so the per-bash-call secret-history `git fsck` stays cheap in time and memory (unbounded growth was a container-OOM contributor). The gate reads `git count-objects -v` (no object-graph walk); gc is best-effort and never fails a backup. Set `0` to disable. |
...
## Repo maintenance (git gc)

After a successful commit, the runner checks the repo's pack count via `git count-objects -v` and, when it reaches `backup.gcPackThreshold` (default 20), runs `git gc`. Because the backup runner (every idle window) and the memory dreaming subagent (every 30 min) commit continuously and nothing else ever repacks or prunes, a long-running agent's repo would otherwise grow unbounded — and the secret-history guard's `git fsck` runs before **every** model bash call, with runtime and memory cost that scale with the object/pack count. Left unchecked this reached 869k objects / 40 packs / 1.4GB in production, where the per-bash fsck's ~850MB RSS contributed to a container OOM.

The gate read does not walk the object graph, so the common quiet backup stays cheap; gc only fires once the threshold is crossed. Maintenance is **best-effort**: it runs under the same `withGitLock` the backup already holds, and any failure (or the count read failing) is logged and swallowed so it can never fail a backup that already committed. Set `backup.gcPackThreshold: 0` to disable.
