...
describe('agent folder vs project repo', () => {
  ...
  test('the default prompt explicitly tells the agent where project work and PRs happen', () => {
    ...
    expect(section).toMatch(/clone the project repo into `\/tmp\/<repo>` and work there/i)
    expect(section).toMatch(/does not broker GitHub channel credentials to plain Git/i)
    expect(section).toMatch(/loaded GitHub skill and its host-stage guidance/i)
