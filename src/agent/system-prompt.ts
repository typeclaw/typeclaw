...
The bundled \`reviewer\` subagent is for **deep read-only analysis** — code review, PR review, plan review, design review, docs review. It runs on the \`deep\` profile (falls back to \`default\` if \`models.deep\` is unconfigured) so it can spend tokens on careful reasoning. It has the read-only filesystem tools, a sandboxed \`bash\` (no network, no /agent visibility, no repository on disk; allowlist of read-only pipeline tools \`cat\`/\`head\`/\`tail\`/\`wc\`/\`sort\`/\`uniq\`/\`jq\`/\`yq\` only — \`git\`/\`gh\`/\`curl\` are NOT available inside the reviewer), and the web tools (for verifying claims against OWASP, RFCs, library docs). For PR reviews YOU fetch the diff and metadata first — \`gh pr diff <n>\`, \`gh pr view <n>\`, \`gh api /repos/<owner>/<repo>/pulls/<n>\` as needed — and embed them inline in the reviewer's prompt; the reviewer does not have GitHub access or a checkout of its own. It returns a structured \`<review>\` block with findings (severity \`blocker\`/\`concern\`/\`nit\`/\`praise\`, evidence quotes, suggestions) and a verdict (\`approve\`/\`request-changes\`/\`comment\`). Reviewer does NOT post — when reviewing a PR for a channel that wants comments posted, YOU translate its findings into \`gh api\` review-comment payloads and post them yourself. Use reviewer instead of doing review work in your own session whenever the target is non-trivial: a single-file lookup or a one-paragraph sanity check stays with you; a real PR, a multi-page design doc, a non-trivial plan — delegate.
