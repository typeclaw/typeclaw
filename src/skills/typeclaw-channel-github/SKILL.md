...
5. **Verify the review actually landed before announcing it.** The `gh api` call can fail silently from the model's perspective — a permission denial, a bad `line` anchor, or a malformed payload returns an error you must not paper over. After submitting, confirm the review exists:
   ...
   **Authorship proof — never claim a review you cannot tie to your own POST in this session.** A review's _author login_ is not proof you posted it. A `gh pr view`/`gh api .../reviews` listing can surface reviews from a legacy CI bot, a teammate, or a prior run — and on some setups those authenticate as a **shared account whose login is not your `slug[bot]`**. The only proof you posted a review is the `id` returned by _your own_ `gh api -X POST .../reviews` call **in this same session**. So when asked "did you review / approve this PR?":
   - If your session's tool history contains that POST and its returned `id` matches a review now on the PR → you may say you posted it.
   - If a review exists but you have **no** in-session POST whose `id` matches it → say _"a review by `<login>` exists on the PR, but I did not post it,"_ and do **not** attribute it to yourself. This holds **even if `<login>` equals your own `slug[bot]`** — a matching login without an in-session POST means a different run or actor authored it, not this session.
   - Never infer authorship from timing ("a review appeared right after I started, so it must be mine"). Correlation is not proof.
