...
- **The PR author is you (`<your-login>`).** You are the **contributor/author** on this PR, not its reviewer. Feedback on it — a review, a "리뷰 반영 부탁드려요" / "address the review" / "please apply the feedback" / "fixed?" comment, a `CHANGES_REQUESTED` someone else left — is yours to **address as the author**: read it, ask the operator for the required host-stage checkout, change and commit the code, then push model-side only with independently exposed credentials or otherwise ask the operator to push from the host stage, and reply. Go to the **contributor flow** ("When the PR is yours"). **You never review your own PR**, and you never spawn the `reviewer` subagent on it — GitHub does not even let an author formally review their own PR, so an attempt produces a self-addressed "review" comment, which is the exact persona-confusion bug to avoid. Gates 1–2 below (review obligations / review requests) describe the _reviewer_ persona and **do not apply when the PR is yours** — skip straight to the contributor flow.
...
2. **Update the PR's branch after a host-stage checkout.** Model-driven bash cannot run `gh pr checkout`, so ask the operator to run the checkout from the host-stage repository directory:

   ```sh
   gh pr checkout <N> --repo owner/repo
   ```

   After the operator confirms the checkout, make the minimal fixes and commit them locally with `typeclaw-git` hygiene. Plain `git push` receives no TypeClaw-brokered GitHub channel credential. It may run model-side only when the operator independently exposed credentials; otherwise ask the operator to push that same branch from the host stage:

   ```sh
   git push
   ```

   Do **not** open a _new_ PR to fix your existing one; whichever push path is available must update the same branch so the open PR updates in place.
...
4. **If CI is failing on your PR**, that's also contributor work: read the failing check, fix the cause on the checked-out branch, and commit locally. Run plain `git push` model-side only when the operator independently exposed credentials; otherwise ask the operator to run it from the host stage. You can't always read another system's CI logs — if you can't, say so and ask for the error rather than guessing.

That's the whole contributor loop: read feedback → operator checks out at the host stage → fix and commit locally → push with independently exposed credentials, or fall back to an operator host-stage push → reply. No subagent, no formal review, no thread resolution.
...
For a pull request, prepare the exact title, body, pushed head branch, and base branch, then ask the operator to run `gh pr create --repo owner/repo --title 'Fix: ...' --head my-branch --base main --body '...'` at the host stage from the repository checkout. Model-driven bash cannot run `gh pr create`. Plain `git push` receives no TypeClaw-brokered GitHub channel credential, though it may use credentials the operator independently exposed; otherwise ask the operator to push from the host stage too.
